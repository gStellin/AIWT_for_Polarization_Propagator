...
//	Definition of the constants
	...
	#define combfilepath "Out_Comb_3_DisjBogDiag_V3.m"
	#define multfilepath "Out_Mult_3_DisjBogDiag_V3.m"

//****************************************************************************
//	Initialization of the arrays

	int AdcbdVertices[8][2][S];
	int MultdcbdVertices[S];
	int AdcbdTemp[8][2][S];
	int MultdcbdTemp[S];
	int Temp[8][2][64];
	int Test[8][2];
	...
	array<array<array<int,2>,8>,S> Adcombbogdiag;	
	...
	#include "In_Comb_3_DisjBogDiag_V3.h"
...
//	Opening titles 
	...
	printf("								 \n");
	printf("    f o r   D i s j o i n t   B o g o l i u b o v   t y p e      \n");
	printf("								 \n");
	printf("       w  i t h   p r o p a g a t o r s    d r e s s e d         \n");
	printf("								 \n");
	printf("               a l o n g   t h e   d i a g o n a l               \n");
	printf("								 \n");
	printf("*****************************************************************\n");
	printf("								 \n");
	...
	for(t=0; t<=5; t++){
		...
		if(t == 0){
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcbdTemp[i][j][u] = Adcombbogdiag[u][i][j];
					};
				};
				MultdcbdTemp[u] = 0;
				MultdcbdVertices[u] = 0;
			};
		}else{
			nS = s;
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcbdTemp[i][j][u] = AdcbdVertices[i][j][u];
					};
				};
				MultdcbdTemp[u] = MultdcbdVertices[u];
				MultdcbdVertices[u] = 0;
			};
		};
  		...
  		s = 1;
  		AdcbdVertices[0][0][0] = AdcbdTemp[0][0][0];
  		AdcbdVertices[0][1][0] = AdcbdTemp[0][1][0];
  		AdcbdVertices[1][0][0] = AdcbdTemp[1][0][0];
  		AdcbdVertices[1][1][0] = AdcbdTemp[1][1][0];
  		AdcbdVertices[2][0][0] = AdcbdTemp[2][0][0];
  		AdcbdVertices[2][1][0] = AdcbdTemp[2][1][0];
  		AdcbdVertices[3][0][0] = AdcbdTemp[3][0][0];
  		AdcbdVertices[3][1][0] = AdcbdTemp[3][1][0];
  		AdcbdVertices[4][0][0] = AdcbdTemp[4][0][0];
  		AdcbdVertices[4][1][0] = AdcbdTemp[4][1][0];
  		AdcbdVertices[5][0][0] = AdcbdTemp[5][0][0];
  		AdcbdVertices[5][1][0] = AdcbdTemp[5][1][0];
  		AdcbdVertices[6][0][0] = AdcbdTemp[6][0][0];
  		AdcbdVertices[6][1][0] = AdcbdTemp[6][1][0];
  		AdcbdVertices[7][0][0] = AdcbdTemp[7][0][0];
  		AdcbdVertices[7][1][0] = AdcbdTemp[7][1][0];
		MultdcbdVertices[0] = max(1,MultdcbdTemp[0]);
...
//	Beginning of the main loop
  		...
  		for(u = 1; u <= nS-1; u++){
   			...
   			for(i = 0; i <= 7; i++){
				for(j = 0; j <= 1; j++){
     					if(AdcbdTemp[i][j][u] == 1){
						i1 = i;
						j1 = j;
					};
     					if(AdcbdTemp[i][j][u] == 2){
						i2 = i;
						j2 = j;
					};
     					if(AdcbdTemp[i][j][u] == 3){
						i3 = i;
						j3 = j;
					};
     					if(AdcbdTemp[i][j][u] == 4){
						i4 = i;
						j4 = j;
					};
     					if(AdcbdTemp[i][j][u] == 5){
						i5 = i;
						j5 = j;
					};
     					if(AdcbdTemp[i][j][u] == 6){
						i6 = i;
						j6 = j;
					};
     					if(AdcbdTemp[i][j][u] == 7){
						i7 = i;
						j7 = j;
					};
     					if(AdcbdTemp[i][j][u] == 8){
						i8 = i;
						j8 = j;
					};
     					if(AdcbdTemp[i][j][u] == 9){
						i9 = i;
						j9 = j;
					};
     					if(AdcbdTemp[i][j][u] == 10){
						i10 = i;
						j10 = j;
					};
     					if(AdcbdTemp[i][j][u] == 11){
						i11 = i;
						j11 = j;
					};
     					if(AdcbdTemp[i][j][u] == 12){
						i12 = i;
						j12 = j;
					};
					Test[i][j] = AdcbdTemp[i][j][u];
 				};
			};
...
//	Sorting of the indices in ascending order from left to right within the pairs
			...
			for(v=0; v<=s-1; v++){
				Avert1[v] = AdcbdVertices[0][0][v];
				Avert2[v] = AdcbdVertices[0][1][v];
				Avert3[v] = AdcbdVertices[1][0][v];
				Avert4[v] = AdcbdVertices[1][1][v];
				Avert5[v] = AdcbdVertices[2][0][v];
				Avert6[v] = AdcbdVertices[2][1][v];
				Avert7[v] = AdcbdVertices[3][0][v];
				Avert8[v] = AdcbdVertices[3][1][v];
				Avert9[v] = AdcbdVertices[4][0][v];
				Avert10[v] = AdcbdVertices[4][1][v];
				Avert11[v] = AdcbdVertices[5][0][v];
				Avert12[v] = AdcbdVertices[5][1][v];
				Avert13[v] = AdcbdVertices[6][0][v];
				Avert14[v] = AdcbdVertices[6][1][v];
				Avert15[v] = AdcbdVertices[7][0][v];
				Avert16[v] = AdcbdVertices[7][1][v];
			};
...
//	compared with the "s" inequivalent fully-contracted terms, already stored. 
			...
			for(v = 0; v<=s-1; v++){
				...
				if(IfRepeat == true){
					MultdcbdVertices[v] = MultdcbdVertices[v] + max(1,MultdcbdTemp[u]);			
					break;
...
//**********************************************************************************************
			...
			if(IfRepeat == false){
				...
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
    						AdcbdVertices[i][j][s-1] = AdcbdTemp[i][j][u];
					};
				};
				MultdcbdVertices[s-1] = max(1,MultdcbdTemp[u]);
			};
...
//	Multiplicity check
	...
	Multiplicity = 0;
	for(u = 0; u <= s-1; u++){
		Multiplicity = Multiplicity + MultdcbdVertices[u];
	};
...
//	Writing of the output files
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombBogDiag[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
		u+1,AdcbdVertices[0][0][u],AdcbdVertices[0][1][u],AdcbdVertices[1][0][u],AdcbdVertices[1][1][u],AdcbdVertices[2][0][u],
		AdcbdVertices[2][1][u],AdcbdVertices[3][0][u],AdcbdVertices[3][1][u],AdcbdVertices[4][0][u],AdcbdVertices[4][1][u],
		AdcbdVertices[5][0][u],AdcbdVertices[5][1][u],AdcbdVertices[6][0][u],AdcbdVertices[6][1][u],AdcbdVertices[7][0][u],
 		AdcbdVertices[7][1][u]);
