...
//	Input parameters 
	...
	#define combfilepath "Out_Comb_3_ConjComp_V3.m"
	#define multfilepath "Out_Mult_3_ConjComp_V3.m"
...
// 	Definition of the arrays
	...
	array<array<array<int,2>,8>,U> Accombcomp1;
	array<array<array<int,2>,8>,V> Accombcomp2;
	array<array<array<int,2>,8>,W> Accombcomp3;

	array<int,U> Multcccvert1;
	array<int,V> Multcccvert2;
	array<int,W> Multcccvert3;	
	...
	#include "In_Comb_3_ConjComp_Compl_V3_p1.h"
	#include "In_Comb_3_ConjComp_Compl_V3_p2.h"
	#include "In_Comb_3_ConjComp_Compl_V3_p3.h"
	#include "In_Mult_3_ConjComp_Compl_V3_p1.h"
	#include "In_Mult_3_ConjComp_Compl_V3_p2.h"
	#include "In_Mult_3_ConjComp_Compl_V3_p3.h"
	...
	for(t=0; t<=5; t++){
		...
		if(t == 0){
			...
			for(u=0; u<=U-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AcccTemp[i][j][u] = Accombcomp1[u][i][j];
					};
				};
				MultcccTemp[u] = Multcccvert1[u];
				MultcccVertices[u] = 0;
			};
			for(u=U; u<=U+V-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AcccTemp[i][j][u] = Accombcomp2[u-U][i][j];
					};
				};
				MultcccTemp[u] = Multcccvert2[u-U];
				MultcccVertices[u] = 0;
			};
			for(u=U+V; u<=U+V+W-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AcccTemp[i][j][u] = Accombcomp3[u-U-V][i][j];
					};
				};
				MultcccTemp[u] = Multcccvert3[u-U-V];
				MultcccVertices[u] = 0;
			};
		}else{
			nS = s;
...
//**********************************************************************************************
//	Beginning of the main loop (fully-contracted terms "u")
...
//**********************************************************************************************
//	Test of equivalence: the fully-contracted term "u" undergoes 64 index permutations, each 
//      of them is compared with the "s" inequivalent fully-contracted terms, already stored. 
...
//	Writing of the ouput files
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombComp[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
