...
// 	Definition of the constants
	...
	#define combfilepath "Out_Comb_3_DisjBogAntidiag_V3.h"
	#define multfilepath "Out_Mult_3_DisjBogAntidiag_V3.h"

//****************************************************************************
//	Initialization of the arrays	

	int AdcbaVertices[8][2][S];
	int MultdcbaVertices[S];
	int AdcbaTemp[8][2][S];
	int MultdcbaTemp[S];
	int Temp[8][2][64];
	int Test[8][2];
	...
	array<array<array<int,2>,8>,S> Adcombbogantidiag;
	...
	#include "In_Comb_3_DisjBogAntidiag_V3.h"
...
// Opening titles 
	...
	printf("								 \n");
	printf("      o f   D i s j o i n t   B o g o l i u b o v   t y p e      \n");
	printf("								 \n");
	printf("       w  i t h   p r o p a g a t o r s    d r e s s e d         \n");
	printf("								 \n");
	printf("           a l o n g   t h e   a n t i d i a g o n a l           \n");
	printf("								 \n");
	printf("*****************************************************************\n");
	printf("								 \n");
	...
	for(t=0; t<=5; t++){
		...
		if(t == 0){
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcbaTemp[i][j][u] = Adcombbogantidiag[u][i][j];
					};
				};
				MultdcbaVertices[u] = 0;
				MultdcbaTemp[u] = 0; 
			};
		}else{
			nS = s;
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcbaTemp[i][j][u] = AdcbaVertices[i][j][u];
					};
				};
				MultdcbaTemp[u] = MultdcbaVertices[u];
				MultdcbaVertices[u] = 0;
			};
		};
  		...
  		s = 1;
  		AdcbaVertices[0][0][0] = AdcbaTemp[0][0][0];
  		AdcbaVertices[0][1][0] = AdcbaTemp[0][1][0];
  		AdcbaVertices[1][0][0] = AdcbaTemp[1][0][0];
  		AdcbaVertices[1][1][0] = AdcbaTemp[1][1][0];
  		AdcbaVertices[2][0][0] = AdcbaTemp[2][0][0];
  		AdcbaVertices[2][1][0] = AdcbaTemp[2][1][0];
  		AdcbaVertices[3][0][0] = AdcbaTemp[3][0][0];
  		AdcbaVertices[3][1][0] = AdcbaTemp[3][1][0];
  		AdcbaVertices[4][0][0] = AdcbaTemp[4][0][0];
  		AdcbaVertices[4][1][0] = AdcbaTemp[4][1][0];
  		AdcbaVertices[5][0][0] = AdcbaTemp[5][0][0];
  		AdcbaVertices[5][1][0] = AdcbaTemp[5][1][0];
  		AdcbaVertices[6][0][0] = AdcbaTemp[6][0][0];
  		AdcbaVertices[6][1][0] = AdcbaTemp[6][1][0];
  		AdcbaVertices[7][0][0] = AdcbaTemp[7][0][0];
  		AdcbaVertices[7][1][0] = AdcbaTemp[7][1][0];
		MultdcbaVertices[0] = max(1,MultdcbaTemp[0]);
...
//	Beginning of the main loop
  		...
  		for(u = 1; u <= nS-1; u++){
   			...
   			for(i = 0; i <= 7; i++){
				for(j = 0; j <= 1; j++){
     					if(AdcbaTemp[i][j][u] == 1){
						i1 = i;
						j1 = j;
					};
     					if(AdcbaTemp[i][j][u] == 2){
						i2 = i;
						j2 = j;
					};
     					if(AdcbaTemp[i][j][u] == 3){
						i3 = i;
						j3 = j;
					};
     					if(AdcbaTemp[i][j][u] == 4){
						i4 = i;
						j4 = j;
					};
     					if(AdcbaTemp[i][j][u] == 5){
						i5 = i;
						j5 = j;
					};
     					if(AdcbaTemp[i][j][u] == 6){
						i6 = i;
						j6 = j;
					};
     					if(AdcbaTemp[i][j][u] == 7){
						i7 = i;
						j7 = j;
					};
     					if(AdcbaTemp[i][j][u] == 8){
						i8 = i;
						j8 = j;
					};
     					if(AdcbaTemp[i][j][u] == 9){
						i9 = i;
						j9 = j;
					};
     					if(AdcbaTemp[i][j][u] == 10){
						i10 = i;
						j10 = j;
					};
     					if(AdcbaTemp[i][j][u] == 11){
						i11 = i;
						j11 = j;
					};
     					if(AdcbaTemp[i][j][u] == 12){
						i12 = i;
						j12 = j;
					};
					Test[i][j] = AdcbaTemp[i][j][u];
 				};
			};
...
//	Sorting of the indices in ascending order from left to right within the pairs
			...
			int *Temp4 = new int[64];
			int *Temp5 = new int[64];
			int *Temp6 = new int[64];
			int *Temp7 = new int[64];
			int *Temp8 = new int[64];
			int *Temp9 = new int[64];
			int *Temp10 = new int[64];
			int *Temp11 = new int[64];
			int *Temp12 = new int[64];
			int *Temp13 = new int[64];
			int *Temp14 = new int[64];
			int *Temp15 = new int[64];
			int *Temp16 = new int[64];
			...
			int *Avert15 = new int[s];
			int *Avert16 = new int[s];
			...
			for(v = 0; v <= s-1; v++){
				Avert1[v] = AdcbaVertices[0][0][v];
				Avert2[v] = AdcbaVertices[0][1][v];
				Avert3[v] = AdcbaVertices[1][0][v];
				Avert4[v] = AdcbaVertices[1][1][v];
				Avert5[v] = AdcbaVertices[2][0][v];
				Avert6[v] = AdcbaVertices[2][1][v];
				Avert7[v] = AdcbaVertices[3][0][v];
				Avert8[v] = AdcbaVertices[3][1][v];
				Avert9[v] = AdcbaVertices[4][0][v];
				Avert10[v] = AdcbaVertices[4][1][v];
				Avert11[v] = AdcbaVertices[5][0][v];
				Avert12[v] = AdcbaVertices[5][1][v];
				Avert13[v] = AdcbaVertices[6][0][v];
				Avert14[v] = AdcbaVertices[6][1][v];
				Avert15[v] = AdcbaVertices[7][0][v];
				Avert16[v] = AdcbaVertices[7][1][v];
...
// 	compared with the "s" inequivalent fully-contracted terms, already stored. 
			...
			for(v = 0; v<=s-1; v++){
				...
				if(IfRepeat == true){
					MultdcbaVertices[v] = MultdcbaVertices[v] + max(1,MultdcbaTemp[u]);
					break;
				};
...
//**********************************************************************************************
			...
			if(IfRepeat == false){
				...
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
    						AdcbaVertices[i][j][s-1] = AdcbaTemp[i][j][u];
					};
				};
				MultdcbaVertices[s-1] = max(1,MultdcbaTemp[u]);
			};
...
//	Multiplicity check
	...
	Multiplicity = 0;
	for(u = 0; u <= s-1; u++){
		Multiplicity = Multiplicity + MultdcbaVertices[u];
	};
...
//	Writing of the output files
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombBogAntidiag[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
		u+1,AdcbaVertices[0][0][u],AdcbaVertices[0][1][u],AdcbaVertices[1][0][u],AdcbaVertices[1][1][u],AdcbaVertices[2][0][u],
		AdcbaVertices[2][1][u],AdcbaVertices[3][0][u],AdcbaVertices[3][1][u],AdcbaVertices[4][0][u],AdcbaVertices[4][1][u],
		AdcbaVertices[5][0][u],AdcbaVertices[5][1][u],AdcbaVertices[6][0][u],AdcbaVertices[6][1][u],AdcbaVertices[7][0][u],
 		AdcbaVertices[7][1][u]);
