...
//**********************************************************************************************
	...
	#define combfilepath  "Out_Comb_3_DisjDirRight_V3.m"
	#define multfilepath  "Out_Mult_3_DisjDirRight_V3.m"

//****************************************************************************

	int AdcdrVertices[8][2][S];
	int MultdcdrVertices[S];
	int AdcdrTemp[8][2][S];
	int MultdcdrTemp[S];
	int Temp[8][2][64];
	int Test[8][2];
	...
	array<array<array<int,2>,8>,S> Adcombdirright;
	...
	#include "In_Comb_3_DisjDirRight_V3.h"
...
// Opening titles 
	...
	printf("								 \n");
	printf("    w i t h   r i g h t - d r e s s e d   p r o p a g a t o r    \n");
	printf("								 \n");
	printf("*****************************************************************\n");
	printf("								 \n");
...
// contractions, together with their multiplicity	
	...
	for(t=0; t<=5; t++){
		...
		if(t == 0){
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcdrTemp[i][j][u] = Adcombdirright[u][i][j];
					};
				};
				MultdcdrTemp[u] = 0;
				MultdcdrVertices[u] = 0;
			};
		}else{
			nS = s;
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcdrTemp[i][j][u] = AdcdrVertices[i][j][u];
					};
				};
				MultdcdrTemp[u] = MultdcdrVertices[u];
				MultdcdrVertices[u] = 0;
			};
		};
  		...
  		s = 1;
  		AdcdrVertices[0][0][0] = AdcdrTemp[0][0][0];
  		AdcdrVertices[0][1][0] = AdcdrTemp[0][1][0];
  		AdcdrVertices[1][0][0] = AdcdrTemp[1][0][0];
  		AdcdrVertices[1][1][0] = AdcdrTemp[1][1][0];
  		AdcdrVertices[2][0][0] = AdcdrTemp[2][0][0];
  		AdcdrVertices[2][1][0] = AdcdrTemp[2][1][0];
  		AdcdrVertices[3][0][0] = AdcdrTemp[3][0][0];
  		AdcdrVertices[3][1][0] = AdcdrTemp[3][1][0];
  		AdcdrVertices[4][0][0] = AdcdrTemp[4][0][0];
  		AdcdrVertices[4][1][0] = AdcdrTemp[4][1][0];
  		AdcdrVertices[5][0][0] = AdcdrTemp[5][0][0];
  		AdcdrVertices[5][1][0] = AdcdrTemp[5][1][0];
  		AdcdrVertices[6][0][0] = AdcdrTemp[6][0][0];
  		AdcdrVertices[6][1][0] = AdcdrTemp[6][1][0];
  		AdcdrVertices[7][0][0] = AdcdrTemp[7][0][0];
  		AdcdrVertices[7][1][0] = AdcdrTemp[7][1][0];
		MultdcdrVertices[0] = max(1,MultdcdrTemp[0]);
...
// Beginning of the main loop
  		...
  		for(u = 1; u <= nS-1; u++){
   			...
   			for(i = 0; i <= 7; i++){
				for(j = 0; j <= 1; j++){
     					if(AdcdrTemp[i][j][u] == 1){
						i1 = i;
						j1 = j;
					};
     					if(AdcdrTemp[i][j][u] == 2){
						i2 = i;
						j2 = j;
					};
     					if(AdcdrTemp[i][j][u] == 3){
						i3 = i;
						j3 = j;
					};
     					if(AdcdrTemp[i][j][u] == 4){
						i4 = i;
						j4 = j;
					};
     					if(AdcdrTemp[i][j][u] == 5){
						i5 = i;
						j5 = j;
					};
     					if(AdcdrTemp[i][j][u] == 6){
						i6 = i;
						j6 = j;
					};
     					if(AdcdrTemp[i][j][u] == 7){
						i7 = i;
						j7 = j;
					};
     					if(AdcdrTemp[i][j][u] == 8){
						i8 = i;
						j8 = j;
					};
     					if(AdcdrTemp[i][j][u] == 9){
						i9 = i;
						j9 = j;
					};
     					if(AdcdrTemp[i][j][u] == 10){
						i10 = i;
						j10 = j;
					};
     					if(AdcdrTemp[i][j][u] == 11){
						i11 = i;
						j11 = j;
					};
     					if(AdcdrTemp[i][j][u] == 12){
						i12 = i;
						j12 = j;
					};
					Test[i][j] = AdcdrTemp[i][j][u];
 				};
			};
...
//	Sorting of the indices in ascending order from left to right within the pairs
			...
			for(v=0; v<=s-1; v++){
				Avert1[v] = AdcdrVertices[0][0][v];
				Avert2[v] = AdcdrVertices[0][1][v];
				Avert3[v] = AdcdrVertices[1][0][v];
				Avert4[v] = AdcdrVertices[1][1][v];
				Avert5[v] = AdcdrVertices[2][0][v];
				Avert6[v] = AdcdrVertices[2][1][v];
				Avert7[v] = AdcdrVertices[3][0][v];
				Avert8[v] = AdcdrVertices[3][1][v];
				Avert9[v] = AdcdrVertices[4][0][v];
				Avert10[v] = AdcdrVertices[4][1][v];
				Avert11[v] = AdcdrVertices[5][0][v];
				Avert12[v] = AdcdrVertices[5][1][v];
				Avert13[v] = AdcdrVertices[6][0][v];
				Avert14[v] = AdcdrVertices[6][1][v];
				Avert15[v] = AdcdrVertices[7][0][v];
				Avert16[v] = AdcdrVertices[7][1][v];
			};
...
//	compared with the "s" inequivalent contractions, already stored. 
			...
			for(v = 0; v<=s-1; v++){
				...
				if(IfRepeat == true){
					MultdcdrVertices[v] = MultdcdrVertices[v] + max(1,MultdcdrTemp[u]);				
					break;
				};
...
//**********************************************************************************************
			...
			if(IfRepeat == false){
				...
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
    						AdcdrVertices[i][j][s-1] = AdcdrTemp[i][j][u];
					};
				};
				MultdcdrVertices[s-1] = max(1,MultdcdrTemp[u]);
			};
...
// 	Multiplicity check
	...
	Multiplicity = 0;
	for(u = 0; u <= s-1; u++){
		Multiplicity = Multiplicity + MultdcdrVertices[u];
	};
...
//	Writing of the output files
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombDirRight[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
		u+1,AdcdrVertices[0][0][u],AdcdrVertices[0][1][u],AdcdrVertices[1][0][u],AdcdrVertices[1][1][u],AdcdrVertices[2][0][u],
		AdcdrVertices[2][1][u],AdcdrVertices[3][0][u],AdcdrVertices[3][1][u],AdcdrVertices[4][0][u],AdcdrVertices[4][1][u],
		AdcdrVertices[5][0][u],AdcdrVertices[5][1][u],AdcdrVertices[6][0][u],AdcdrVertices[6][1][u],AdcdrVertices[7][0][u],
 		AdcdrVertices[7][1][u]);
