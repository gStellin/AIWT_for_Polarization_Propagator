...
//	Definition of the constants
	...
	#define combfilepath "Out_Comb_3_ConjSkel_V3.m"
	#define multfilepath "Out_Mult_3_ConjSkel_V3.m"

//****************************************************************************
//	Initialization of the arrays

	int AccsVertices[8][2][S];
	int MultccsVertices[S];
	int AccsTemp[8][2][S];
	int MultccsTemp[S];
	int Temp[8][2][64];
	int Test[8][2];

	array<array<array<int,2>,8>,S> Accombskel;	
	...
	#include "In_Comb_3_ConjSkel_V3.h"
...
//	Opening titles 
	...
	printf("								 \n");
	printf("      o f  C o n j o i n t   S k e l e t o n   t y p e           \n");
	printf("								 \n");
	printf("*****************************************************************\n");
	printf("								 \n");
	...
	for(t=0; t<= 5; t++){
		...
		if(t == 0){
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AccsTemp[i][j][u] = Accombskel[u][i][j];
					};
				};
				MultccsTemp[u] = 0;
				MultccsVertices[u] = 0;
			...
			for(u=0; u<=S-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AccsTemp[i][j][u] = AccsVertices[i][j][u];
					};
				};
				MultccsTemp[u] = MultccsVertices[u];
				MultccsVertices[u] = 0;
			};
		};
  		...
  		s = 1;
  		AccsVertices[0][0][0] = AccsTemp[0][0][0];
  		AccsVertices[0][1][0] = AccsTemp[0][1][0];
  		AccsVertices[1][0][0] = AccsTemp[1][0][0];
  		AccsVertices[1][1][0] = AccsTemp[1][1][0];
  		AccsVertices[2][0][0] = AccsTemp[2][0][0];
  		AccsVertices[2][1][0] = AccsTemp[2][1][0];
  		AccsVertices[3][0][0] = AccsTemp[3][0][0];
  		AccsVertices[3][1][0] = AccsTemp[3][1][0];
  		AccsVertices[4][0][0] = AccsTemp[4][0][0];
  		AccsVertices[4][1][0] = AccsTemp[4][1][0];
  		AccsVertices[5][0][0] = AccsTemp[5][0][0];
  		AccsVertices[5][1][0] = AccsTemp[5][1][0];
  		AccsVertices[6][0][0] = AccsTemp[6][0][0];
  		AccsVertices[6][1][0] = AccsTemp[6][1][0];
  		AccsVertices[7][0][0] = AccsTemp[7][0][0];
  		AccsVertices[7][1][0] = AccsTemp[7][1][0];
		MultccsVertices[0] = max(1,MultccsTemp[0]);
   			...
   			for(i = 0; i <= 7; i++){
				for(j = 0; j <= 1; j++){
						...
						j9 = j;
     					...
     					if(AccsTemp[i][j][u] == 10){
						i10 = i;
						j10 = j;
					};
     					if(AccsTemp[i][j][u] == 11){
						i11 = i;
						j11 = j;
					};
     					if(AccsTemp[i][j][u] == 12){
						i12 = i;
						j12 = j;
					};
					Test[i][j] = AccsTemp[i][j][u];
 				};
			};
...
//	Sorting of the indices in ascending order from left to right within the pairs
			...
			for(v=0; v<=s-1; v++){
				Avert1[v] = AccsVertices[0][0][v];
				Avert2[v] = AccsVertices[0][1][v];
				Avert3[v] = AccsVertices[1][0][v];
				Avert4[v] = AccsVertices[1][1][v];
				Avert5[v] = AccsVertices[2][0][v];
				Avert6[v] = AccsVertices[2][1][v];
				Avert7[v] = AccsVertices[3][0][v];
				Avert8[v] = AccsVertices[3][1][v];
				Avert9[v] = AccsVertices[4][0][v];
				Avert10[v] = AccsVertices[4][1][v];
				Avert11[v] = AccsVertices[5][0][v];
				Avert12[v] = AccsVertices[5][1][v];
				Avert13[v] = AccsVertices[6][0][v];
				Avert14[v] = AccsVertices[6][1][v];
				Avert15[v] = AccsVertices[7][0][v];
				Avert16[v] = AccsVertices[7][1][v];
			};
...
// 	compared with the "s" inequivalent fully-contracted terms, already stored. 
			...
			for(v = 0; v<=s-1; v++){
				...
				if(IfRepeat == true){
					MultccsVertices[v] = MultccsVertices[v] + max(1,MultccsTemp[u]);
					break;
				};
...
//**********************************************************************************************
			...
			if(IfRepeat == false){
				...
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
    						AccsVertices[i][j][s-1] = AccsTemp[i][j][u];
					};
				};
				MultccsVertices[s-1] = max(1,MultccsTemp[u]);
			};
...
//**********************************************************************************************
	...
	Multiplicity = 0;
	for(u = 0; u <= s-1; u++){
		Multiplicity = Multiplicity + MultccsVertices[u];
	};
...
//	Generation of the output files 
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombSkel[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
		u+1,AccsVertices[0][0][u],AccsVertices[0][1][u],AccsVertices[1][0][u],AccsVertices[1][1][u],AccsVertices[2][0][u],
		AccsVertices[2][1][u],AccsVertices[3][0][u],AccsVertices[3][1][u],AccsVertices[4][0][u],AccsVertices[4][1][u],
		AccsVertices[5][0][u],AccsVertices[5][1][u],AccsVertices[6][0][u],AccsVertices[6][1][u],AccsVertices[7][0][u],
 		AccsVertices[7][1][u]);
