...
//	Definition of the constants
	...
	#define combfilepath "Out_Comb_3_DisjBogoliubov_V3.m"
	#define multfilepath "Out_Mult_3_DisjBogoliubov_V3.m"

//***********************************************************************
//	Initialization of the arrays

	int AdcbVertices[8][2][S];
	int MultdcbVertices[S];
	int AdcbTemp[8][2][S];
	int MultdcbTemp[S];
	int Temp[8][2][64];
	int Test[8][2];
	...
	array<array<array<int,2>,8>,S> Adcombbog;
	...
	#include "In_Comb_3_DisjBogoliubov_V3.h"
	...
	for(t=0; t<=5; t++){
		...
		if(t == 0){
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcbTemp[i][j][u] = Adcombbog[u][i][j];
					};
				};
				MultdcbTemp[u] = 0;
				MultdcbVertices[u] = 0;
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcbTemp[i][j][u] = AdcbVertices[i][j][u];
					};
				};
				MultdcbTemp[u] = MultdcbVertices[u];
				MultdcbVertices[u] = 0;
			};
		};
  		...
  		s = 1;
  		AdcbVertices[0][0][0] = AdcbTemp[0][0][0];
  		AdcbVertices[0][1][0] = AdcbTemp[0][1][0];
  		AdcbVertices[1][0][0] = AdcbTemp[1][0][0];
  		AdcbVertices[1][1][0] = AdcbTemp[1][1][0];
  		AdcbVertices[2][0][0] = AdcbTemp[2][0][0];
  		AdcbVertices[2][1][0] = AdcbTemp[2][1][0];
  		AdcbVertices[3][0][0] = AdcbTemp[3][0][0];
  		AdcbVertices[3][1][0] = AdcbTemp[3][1][0];
  		AdcbVertices[4][0][0] = AdcbTemp[4][0][0];
  		AdcbVertices[4][1][0] = AdcbTemp[4][1][0];
  		AdcbVertices[5][0][0] = AdcbTemp[5][0][0];
  		AdcbVertices[5][1][0] = AdcbTemp[5][1][0];
  		AdcbVertices[6][0][0] = AdcbTemp[6][0][0];
  		AdcbVertices[6][1][0] = AdcbTemp[6][1][0];
  		AdcbVertices[7][0][0] = AdcbTemp[7][0][0];
  		AdcbVertices[7][1][0] = AdcbTemp[7][1][0];
		MultdcbVertices[0] = max(1,MultdcbTemp[0]);
   			...
   			for(i = 0; i <= 7; i++){
				for(j = 0; j <= 1; j++){
						...
						j9 = j;
     					...
     					if(AdcbTemp[i][j][u] == 10){
						i10 = i;
						j10 = j;
					};
     					if(AdcbTemp[i][j][u] == 11){
						i11 = i;
						j11 = j;
					};
     					if(AdcbTemp[i][j][u] == 12){
						i12 = i;
						j12 = j;
					};
					Test[i][j] = AdcbTemp[i][j][u];
 				};
			};
...
//	Sorting of the indices in ascending order from left to right within the pairs
			...
			for(v=0; v<=s-1; v++){
				Avert1[v] = AdcbVertices[0][0][v];
				Avert2[v] = AdcbVertices[0][1][v];
				Avert3[v] = AdcbVertices[1][0][v];
				Avert4[v] = AdcbVertices[1][1][v];
				Avert5[v] = AdcbVertices[2][0][v];
				Avert6[v] = AdcbVertices[2][1][v];
				Avert7[v] = AdcbVertices[3][0][v];
				Avert8[v] = AdcbVertices[3][1][v];
				Avert9[v] = AdcbVertices[4][0][v];
				Avert10[v] = AdcbVertices[4][1][v];
				Avert11[v] = AdcbVertices[5][0][v];
				Avert12[v] = AdcbVertices[5][1][v];
				Avert13[v] = AdcbVertices[6][0][v];
				Avert14[v] = AdcbVertices[6][1][v];
				Avert15[v] = AdcbVertices[7][0][v];
				Avert16[v] = AdcbVertices[7][1][v];
			};
...
//	compared with the "s" inequivalent fully-contracted terms, already stored. 
			...
			for(v = 0; v<=s-1; v++){
				...
				if(IfRepeat == true){
					MultdcbVertices[v] = MultdcbVertices[v] + max(1,MultdcbTemp[u]);				
					break;
...
//**********************************************************************************************
			...
			if(IfRepeat == false){
				...
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
    						AdcbVertices[i][j][s-1] = AdcbTemp[i][j][u];
					};
				};
				MultdcbVertices[s-1] = max(1,MultdcbTemp[u]);
			};
...
//	Multiplicity check
	...
	Multiplicity = 0;
	for(u = 0; u <= s-1; u++){
		Multiplicity = Multiplicity + MultdcbVertices[u];
	};
...
//	Writing of the output files
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombBog[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
		u+1,AdcbVertices[0][0][u],AdcbVertices[0][1][u],AdcbVertices[1][0][u],AdcbVertices[1][1][u],AdcbVertices[2][0][u],
		AdcbVertices[2][1][u],AdcbVertices[3][0][u],AdcbVertices[3][1][u],AdcbVertices[4][0][u],AdcbVertices[4][1][u],
		AdcbVertices[5][0][u],AdcbVertices[5][1][u],AdcbVertices[6][0][u],AdcbVertices[6][1][u],AdcbVertices[7][0][u],
 		AdcbVertices[7][1][u]);
