...
//	Definition of the constants
	...
	#define combfilepath "Out_Comb_3_Disconnected_V3.m"
	#define multfilepath "Out_Mult_3_Disconnected_V3.m"

//****************************************************************************

	int AdVertices[8][2][S];
	int MultdVertices[S];
	int AdTemp[8][2][S];
	int MultdTemp[S];
	int Temp[8][2][64];
	...
	array<array<array<int,2>,8>,S> Acombdisc;
	...
	#include "In_Comb_3_Disconnected_V3.h"
...
//	Opening titles 
	...
	printf("								 \n");
	printf("	    o f   f u l l y - c o n t r a c t e d 		 \n");
	printf("								 \n");
...
//	fully-contracted terms, together with their multiplicity
	...
	for(t = 0; t <= 5; t++){
		...
		if(t == 0){
  		...
  		AdVertices[7][0][0] = AdTemp[7][0][0];
  		AdVertices[7][1][0] = AdTemp[7][1][0];
...
//**********************************************************************************************
			...
			for(v=0; v<=s-1; v++){
				...
				Avert15[v] = AdVertices[7][0][v];
				Avert16[v] = AdVertices[7][1][v];
			}
		
			IfRepeat = false;

//**********************************************************************************************
//	Test of equivalence: the contraction "u" undergoes 64 index permutations, each of them is 
...
//	Writing of the output files
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombDisc[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
		...
		AdVertices[5][0][u],AdVertices[5][1][u],AdVertices[6][0][u],AdVertices[6][1][u],AdVertices[7][0][u],
 		AdVertices[7][1][u]);
