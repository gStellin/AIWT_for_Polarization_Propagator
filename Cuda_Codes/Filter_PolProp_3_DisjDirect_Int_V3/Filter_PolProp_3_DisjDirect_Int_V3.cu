...
//	Definition of the constants
	...
	#define combfilepath "Out_Comb_3_DisjDir_V3.m"
	#define multfilepath "Out_Mult_3_DisjDir_V3.m"

//****************************************************************************
//	Initialization of the arrays

	int AdcdVertices[8][2][S];
	int MultdcdVertices[S];
	int AdcdTemp[8][2][S];
	int MultdcdTemp[S];
	int Temp[8][2][64];
	int Test[8][2];
	...
	array<array<array<int,2>,8>,S> Adcombdir;
	...
	#include "In_Comb_3_DisjDirect_V3.h"
	...
	for(t = 0; t <= 5; t++){
		...
		if(t == 0){
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcdTemp[i][j][u] = Adcombdir[u][i][j];
					};
				};
				MultdcdTemp[u] = 0;
				MultdcdVertices[u] = 0;
			...
			for(u=0; u<=nS-1; u++){
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
						AdcdTemp[i][j][u] = AdcdVertices[i][j][u];
					};
				};
				MultdcdTemp[u] = MultdcdVertices[u];
				MultdcdVertices[u] = 0;
			};
		};
  		...
  		s = 1;
  		AdcdVertices[0][0][0] = AdcdTemp[0][0][0];
  		AdcdVertices[0][1][0] = AdcdTemp[0][1][0];
  		AdcdVertices[1][0][0] = AdcdTemp[1][0][0];
  		AdcdVertices[1][1][0] = AdcdTemp[1][1][0];
  		AdcdVertices[2][0][0] = AdcdTemp[2][0][0];
  		AdcdVertices[2][1][0] = AdcdTemp[2][1][0];
  		AdcdVertices[3][0][0] = AdcdTemp[3][0][0];
  		AdcdVertices[3][1][0] = AdcdTemp[3][1][0];
  		AdcdVertices[4][0][0] = AdcdTemp[4][0][0];
  		AdcdVertices[4][1][0] = AdcdTemp[4][1][0];
  		AdcdVertices[5][0][0] = AdcdTemp[5][0][0];
  		AdcdVertices[5][1][0] = AdcdTemp[5][1][0];
  		AdcdVertices[6][0][0] = AdcdTemp[6][0][0];
  		AdcdVertices[6][1][0] = AdcdTemp[6][1][0];
  		AdcdVertices[7][0][0] = AdcdTemp[7][0][0];
  		AdcdVertices[7][1][0] = AdcdTemp[7][1][0];
		MultdcdVertices[0] = max(1,MultdcdTemp[0]);
   			...
   			for(i = 0; i <= 7; i++){
				for(j = 0; j <= 1; j++){
						...
						j9 = j;
     					...
     					if(AdcdTemp[i][j][u] == 10){
						i10 = i;
						j10 = j;
					};
     					if(AdcdTemp[i][j][u] == 11){
						i11 = i;
						j11 = j;
					};
     					if(AdcdTemp[i][j][u] == 12){
						i12 = i;
						j12 = j;
					};
					Test[i][j] = AdcdTemp[i][j][u];
 				};
			};
...
//**********************************************************************************************
			...
			for(v=0; v<=s-1; v++){
				Avert1[v] = AdcdVertices[0][0][v];
				Avert2[v] = AdcdVertices[0][1][v];
				Avert3[v] = AdcdVertices[1][0][v];
				Avert4[v] = AdcdVertices[1][1][v];
				Avert5[v] = AdcdVertices[2][0][v];
				Avert6[v] = AdcdVertices[2][1][v];
				Avert7[v] = AdcdVertices[3][0][v];
				Avert8[v] = AdcdVertices[3][1][v];
				Avert9[v] = AdcdVertices[4][0][v];
				Avert10[v] = AdcdVertices[4][1][v];
				Avert11[v] = AdcdVertices[5][0][v];
				Avert12[v] = AdcdVertices[5][1][v];
				Avert13[v] = AdcdVertices[6][0][v];
				Avert14[v] = AdcdVertices[6][1][v];
				Avert15[v] = AdcdVertices[7][0][v];
				Avert16[v] = AdcdVertices[7][1][v];
			}
...
// 	compared with the "s" inequivalent fully-contracted terms, already stored. 
			...
			for(v = 0; v <= s-1; v++){
				...
				if(IfRepeat == true){
					MultdcdVertices[v] = MultdcdVertices[v] + max(1,MultdcdTemp[u]);
					break;
				};
...
//**********************************************************************************************
			...
			if(IfRepeat == false){
				...
				for(i=0; i<=7; i++){
					for(j=0; j<=1; j++){
    						AdcdVertices[i][j][s-1] = AdcdTemp[i][j][u];
					};
				};
				MultdcdVertices[s-1] = max(1,MultdcdTemp[u]);
			};
...
//	Multiplicity check
	...
	Multiplicity = 0;
	for(u = 0; u <= s-1; u++){
		Multiplicity = Multiplicity + MultdcdVertices[u];
	};
...
//	Writing of the output files
	...
	for(u = 0; u <= s-1; u++){
		...
		fprintf(CombFile,"CombDir[[%ld]] = {{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d},{%d,%d}};\n",
		u+1,AdcdVertices[0][0][u],AdcdVertices[0][1][u],AdcdVertices[1][0][u],AdcdVertices[1][1][u],AdcdVertices[2][0][u],
		AdcdVertices[2][1][u],AdcdVertices[3][0][u],AdcdVertices[3][1][u],AdcdVertices[4][0][u],AdcdVertices[4][1][u],
		AdcdVertices[5][0][u],AdcdVertices[5][1][u],AdcdVertices[6][0][u],AdcdVertices[6][1][u],AdcdVertices[7][0][u],
 		AdcdVertices[7][1][u]);
